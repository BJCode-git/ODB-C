...
sudo-g5k python3 ./scripts/monitor-usage.py "$CPU_MEASURE_DIR/logs" "$PATTERN" --period $INTERVAL --duration $DURATION --pids $PIDS --pin 0
#wait $!
...
sudo-g5k python3 scripts/plot-cpu.py "$CPU_MEASURE_DIR/logs/" "$CPU_MEASURE_DIR/plots/" --window 5
