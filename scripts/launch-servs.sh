...
# 12288
make clean-lib
make ONE_SHOT="1" USE_ODB="$use_odb" BUFF_SIZE=16384 USE_STANDALONE=0 TYPE="img" run-front-end >> /dev/null &
make ONE_SHOT="1" USE_ODB="$use_odb" BUFF_SIZE=16384 USE_STANDALONE=0 TYPE="img" run-inter >> /dev/null &
make ONE_SHOT="1" USE_ODB="$use_odb" BUFF_SIZE=16384 USE_STANDALONE=0 TYPE="img" run-back-end >> /dev/null &
