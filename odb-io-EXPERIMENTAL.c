...
static void setup_send_virtual_transmit(ODB_Frame *frame,ConnectionInfo *info){
    ...
    (*frame)[ODB_frame_tail].iov_base   = (void*) ((uint8_t*) info->payload.buffer + MIN( info->odb_header.total_size - (*frame)[ODB_frame_tail].iov_len, desc_len - info->desc.d_desc.tail_size));
...
static void setup_send_virtual(ODB_Frame *frame,ConnectionInfo *info){
    ...
    if( info->bytes_read_write < ODB_HEADER_SIZE + ODB_DESC_SIZE)
        ...
        uint8_t *      tail            = (uint8_t *) info->payload.tail   + info->payload.tail_size;
        uint8_t *      end_of_head     = (uint8_t *) info->payload.buffer + info->payload.head_size;
        (*frame)[ODB_frame_tail].iov_base = (void*) ((uint8_t*) ( MAX(tail - (*frame)[ODB_frame_tail].iov_len, end_of_head) ) + td_off);
...
static ODB_ERROR send_virtual(int sockfd,ConnectionInfo *info,int flags, size_t *bytes_written){
    ...
    *bytes_written = (size_t) ret;
    ...
    DEBUG_LOG("Wrote unaligned %zu bytes / %zu total bytes",*bytes_written - (ODB_HEADER_SIZE + ODB_DESC_SIZE) ,should_have_written + info->desc.d_desc.body_size);
